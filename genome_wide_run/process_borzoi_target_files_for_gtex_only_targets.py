...
borzoi_gtex_only_target_file = sys.argv[2]
...
gtex_sample_id_to_individual_tissue_format = create_mapping_from_gtex_sample_id_to_individual_tissue_format(gtex_sample_attributes_file)



t = open(borzoi_gtex_only_target_file,'w')
t.write('orig_target_index\tgtex_only_target_index\ttarget_identifier\ttarget_description\tGTEx_tissue\n')
new_index = 0
for ii, identifier in enumerate(borzoi_target_identifiers):
	...
	t.write(str(ii) + '\t' + str(new_index) + '\t' + identifier + '\t' + borzoi_target_descriptions[ii] + '\t' + tissue_name + '\n')
